...
public class Jogador {
    ...
    private final float MAX_VELOCIDADE = 800f;
    private final float ACELERACAO_POR_CLIQUE = 100f;
    private final float DESACELERACAO = 3.0f;
    public float velocidadeAtual = 0;

    // ESTATÍSTICA CORRIGIDA: Agora pública e inicializada
    ...
    public Jogador(float inicioX, float inicioY, float tamanhoSprite, float mundoLargura) {
        this.x = inicioX;
        this.y = inicioY;
    ...
    public void atualizar(float delta) {
        ...
        // 1. Aplica Movimento e Desaceleração
        ...
        // Lógica de Parado / Espelhamento
        if (velocidadeAtual <= 0.1f) {
            frameBase = animacaoCorrida.getKeyFrames()[0]; // Frame parado
        }

        // Garante que o jogador esteja virado para a direita (sempre)
        if (!viradoDireita && !frameFinal.isFlipX()) {
            frameFinal.flip(true, false);
        } else if (viradoDireita && frameFinal.isFlipX()) {
            frameFinal.flip(true, false);
        }
    ...
    public void inputClique() {
        if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) {
            // CORREÇÃO: Incrementa a variável pública
            cliquesBarraEspaco++;

            if(velocidadeAtual < MAX_VELOCIDADE) {
                velocidadeAtual += ACELERACAO_POR_CLIQUE;
            }
